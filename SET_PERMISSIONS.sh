...
# ============================================
# استخدام: chmod +x SET_PERMISSIONS.sh && ./SET_PERMISSIONS.sh
# أو: bash SET_PERMISSIONS.sh
...
# ============================================
echo -e "${YELLOW}⚙️  ضبط تصريحات الملفات القابلة للتنفيذ...${NC}"
...
if [ -d "$PROJECT_ROOT/uploads" ]; then
    ...
    chown -R www-data:www-data "$PROJECT_ROOT/uploads" 2>/dev/null || echo "⚠️  تأكد من ضبط ملكية مجلد uploads يدوياً"
...
if [ -d "$PROJECT_ROOT/data" ]; then
    ...
    chown -R www-data:www-data "$PROJECT_ROOT/data" 2>/dev/null || echo "⚠️  تأكد من ضبط ملكية مجلد data يدوياً"
...
if [ -d "$PROJECT_ROOT/logs" ]; then
    ...
    chown -R www-data:www-data "$PROJECT_ROOT/logs" 2>/dev/null || echo "⚠️  تأكد من ضبط ملكية مجلد logs يدوياً"
...
if [ -f "$PROJECT_ROOT/.env" ]; then
    ...
    chown www-data:www-data "$PROJECT_ROOT/.env" 2>/dev/null || echo "⚠️  تأكد من ضبط ملكية .env يدوياً"
...
# ============================================
# 7. ضبط ملكية الملفات (www-data)
# ============================================
echo -e "${YELLOW}⚙️  ضبط ملكية الملفات...${NC}"
# التحقق من وجود المستخدم
if id "www-data" &>/dev/null; then
    chown -R www-data:www-data "$PROJECT_ROOT" 2>/dev/null || echo "⚠️  تأكد من تشغيل الأمر كـ root"
    echo -e "${GREEN}✅ تم ضبط الملكية إلى www-data:www-data${NC}"
elif id "apache" &>/dev/null; then
    chown -R apache:apache "$PROJECT_ROOT" 2>/dev/null || echo "⚠️  تأكد من تشغيل الأمر كـ root"
    echo -e "${GREEN}✅ تم ضبط الملكية إلى apache:apache${NC}"
else
    echo -e "${YELLOW}⚠️  لم يتم العثور على www-data أو apache، اضبط الملكية يدوياً${NC}"
fi
